async function openCacheStorage() {
    try {
      return await window.caches.open("ShadcnBlazor.Docs")
    }
    catch (err) {
        return undefined
    }
...
export async function put(url, method, body = "", responseString) {
    ...
    const cachedResponseFields = {
        headers: { 'blaze-fusion-cache-expires': expires.toUTCString() },
    }
...
export async function get(url, method, body = "") {
    ...
    if (response == null) {
        ...
        const expirationDate = Date.parse(response.headers.get('blaze-fusion-cache-expires'))
        const now = new Date()
