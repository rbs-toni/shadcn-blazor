...
public partial class Autocomplete<TOption> : ListComponentBase<TOption> where TOption : notnull
{
    ...
    internal const string JS_FILE = "./_content/ShadcnBlazor/Components/List/Autocomplete.razor.js";

    public Input? Element { get; set; } = default!;
    Virtualize<TOption>? VirtualizationContainer { get; set; }
    readonly Debounce _debounce = new();
    bool _shouldRender = true;

    /// <summary>
    /// Initializes a new instance of the <see cref="Autocomplete{TOption}"/> class.
    /// </summary>
    public Autocomplete()
    {
        Multiple = true;
    ...
    IJSObjectReference Module { get; set; } = default!;
    ...
    /// For the Autocomplete component, use the <see cref="ValueText"/> property instead.
    /// </summary>
    [Parameter]
    [Obsolete]
...
#pragma warning restore CS0809 // Obsolete member overrides non-obsolete member
    ...
    /// <summary>
    /// For <see cref="Autocomplete{TOption}"/>, this property must be True.
    /// Set the <see cref="MaximumSelectedOptions"/> property to 1 to select just one item.
    /// </summary>
    public override bool Multiple
    {
        get { return base.Multiple; }

        set
        {
            if (value == false)
            {
                throw new ArgumentException("For Autocomplete, this property must be True. Set the MaximumSelectedOptions property to 1 to select just one item.");
            }
    ...
    /// <summary>
    /// Gets or sets the style applied to all <see cref="ListOption{TOption}"/> of the component.
    /// </summary>
    [Parameter]
    public string? OptionStyle { get; set; }

    /// <summary>
    /// Gets or sets the css class applied to all <see cref="ListOption{TOption}"/> of the component.
    /// </summary>
    [Parameter]
    ...
    /// </summary>
    [Parameter]
    public IconName? IconDismiss { get; set; } = IconName.X;

    /// <summary>
    /// Gets or sets the icon used for the Search button. By default: Search icon.
    /// </summary>
    [Parameter]
    public IconName? IconSearch { get; set; } = IconName.Search;
    ...
    /// <summary />
    string? ListStyleValue => new StyleBuilder()
    ...
    /// <summary />
    string IdScroll => $"{Id}-scroll";

    /// <summary />
    string IdPopup => $"{Id}-popup";

    /// <summary />
    bool IsMultiSelectOpened { get; set; } = false;

    /// <summary />
    bool IsReachedMaxItems { get; set; } = false;

    /// <summary />
    TOption? SelectableItem { get; set; }
    ...
    ValueTask<ItemsProviderResult<TOption>> LoadFilteredItemsAsync(ItemsProviderRequest request)
    {
        if (Items is null)
        {
    ...
    static readonly KeyCode[] CatchOnly = new[] { KeyCode.Escape, KeyCode.Enter, KeyCode.Backspace, KeyCode.Down, KeyCode.Up };
    static readonly KeyCode[] PreventOnly = CatchOnly.Except(new[] { KeyCode.Backspace }).ToArray();
    static readonly KeyCode[] SelectValueOnTabOnly = new[] { KeyCode.Tab };

    /// <summary />
    protected async Task KeyDownHandlerAsync(KeyCodeEventArgs e)
    {
        switch (e.Key)
        {
            case KeyCode.Escape:
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            Module = await JS.InvokeAsync<IJSObjectReference>("import", JS_FILE);
        }
    }
    ...
    /// <summary />
    async Task DisplayLastSelectedItemAsync()
    {
        if (Module != null)
        {
    ...
    /// <summary />
    string? GetAutocompleteAriaLabel()
    {
        // No items found
    ...
    /// <summary />
    async Task RaiseValueTextChangedAsync(string value)
    {
