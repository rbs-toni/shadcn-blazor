...
public static partial class StringExtensions
{
    ...
    public static string GenerateSlug(this string value, uint? maxLength = null)
    {
        ...
        string result = value.RemoveDiacritics().Replace("-", " ").ToLowerInvariant();
    ...
    /// Examples:
    /// <list type="bullet">
    /// <item><description><c>"HelloWorld"</c> → <c>"hello-world"</c></description></item>
    /// <item><description><c>"thisIsATest"</c> → <c>"this-is-a-test"</c></description></item>
    /// <item><description><c>"Already-Kebab-Case"</c> → <c>"already-kebab-case"</c></description></item>
    /// </list>
    /// </remarks>
    /// </summary>
    [DebuggerStepThrough]
    public static string ToKebabCase(this string input)
    {
        ...
        return HyphenCollapseReducerReg().Replace(normalized, "-")
                    .ToLowerInvariant()
                    .Trim('-');
    }

    [GeneratedRegex(@"([a-z0-9])([A-Z])")]
    private static partial Regex PascalCaseBoundarySplitterReg();
    [GeneratedRegex("-{2,}")]
    private static partial Regex HyphenCollapseReducerReg();
}
